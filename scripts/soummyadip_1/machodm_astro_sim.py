...
runtag = '%s_%s_plawidx_%.2f_zsmax_%.1f_y0_%.1f_%s_%s_Nsim_%.0e' %(zs_dist,
...
path_plot = '../../sim_data/Soummyadip_1/plots/'
...
plt.tight_layout()
plt.savefig(path_plot+'optical_depth_vs_zs_y0_%d_%s.pdf' %(y0, runtag))
plt.close()
...
for zdist in ['uniform', 'Dominik', 'Belczynski']:
...
def calc_lensing_frac_quad(zs_dist, z_max, y0, f_dm): 
    """ calculate the fraction of lensed mergers"""
    return quad(calc_lensing_frac_integrant,0,z_max,args=(zs_dist, z_max, y0, f_dm))[0]
...
z_pdf_loc='../../data/redshift_data/'
z_pdf_data=np.genfromtxt(z_pdf_loc+'/'+'z_PDF.dat',dtype=None,names=True)
...
def calc_snr(m1, m2, spin1z, spin2z, dL, ra, dec, pol, iota, phi0, f_low, f_upp, df, t0_gps, approx): 
    ...
    # generate waveform 
    hp, hc = get_fd_waveform(approximant=approx, mass1=m1, mass2=m2, distance=dL, spin1z=spin1z, 
    ...
    for i_det, det_obj in enumerate(det_obj_list):
        ...
        h = Fp*hp + Fc*hc
        snr[i_det] = filter.matchedfilter.sigma(h, psd=psd_list[i_det], low_frequency_cutoff=f_low, 
                                                high_frequency_cutoff=f_upp)
...
t0_gps = np.random.uniform(1230336018, 1261872018, N_sim)
...
# generate PSDs. FIXME 
#Sh_aLIGO = psd.from_string('aLIGOZeroDetHighPower', length=int(f_upp/df)+1, delta_f=df, low_freq_cutoff=f_low)
#Sh_aVirgo = psd.from_string('AdvVirgo', length=int(f_upp/df)+1, delta_f=df, low_freq_cutoff=f_low)

# create PSD objects - global variables 
#psd_H1 = Sh_aLIGO 
#psd_L1 = Sh_aLIGO 
#psd_V1 = Sh_aVirgo 
...
#     plt.ylim(0,2)
#     plt.tight_layout()
#     plt.savefig('lens_redshift_dist_from_sim_%s_fdm_%.1f.png' %(runtag, f_dm), dpi=300)
#     plt.close()
...
# save the data 
outfile = '../../sim_data/Soummyadip_1/astro_sim_data/machodm_lensing_astro_sim_%s' %(runtag)
...
outfile = '../../sim_data/Soummyadip_1/astro_sim_data/machodm_lensing_astro_sim_%s_lw' %(runtag)
