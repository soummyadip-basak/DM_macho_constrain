...
#                    help='dimensionless source position')
#parser.add_argument('--f_dm', metavar='d', type=float, 
#                    help='fraction of dark matter in the form of MACHOs')
parser.add_argument('--m_i', metavar='m_i', type=float, 
                    help='lens mass (M_sun)')              # i := injected, r := recovery
parser.add_argument('--y_i', metavar='y_i', type=float, 
                    help='dimensionless source position')
parser.add_argument('--m_r', metavar='m_r', type=float, 
                    help='lens mass (M_sun)')
parser.add_argument('--y_r', metavar='y_r', type=float, 
                    help='dimensionless source position')
...
def calc_mismatch(h, approx, mchirp, eta, spin1z, spin2z, df, f_low, f_upp, Sh):
...
#    if isnan(m1) or isnan(m2) or isnan(eff_spin) or m1 < 2. or m2 < 2 or m1/m2 < 1./18 or m1/m2 > 18 or m1+m2 > 800\
#    or eff_spin < -0.99 or eff_spin > 0.99:
    if isnan(m1) or isnan(m2) or isnan(spin1z) or isnan(spin2z) or m1 < 2. or m2 < 2 or m1/m2 < 1./18 or m1/m2 > 18 or m1+m2 > 800\
    or spin1z < -0.99 or spin1z > 0.99 or spin2z < -0.99 or spin2z > 0.99:
        log_mismatch = 1e6   
    else: 
        # generate template waveform 
...
def hp_hc(approx, mchirp, eta, spin1z, spin2z, f_low, f_upp, Sh, f_Sh, M_lz=m_i, y_l=y_i, intrp_hyp1f1=None):
    mtot = mchirp * np.power(eta, -3./5)
    fac = np.sqrt(1. - 4.*eta)
    m1, m2 = (mtot * (1. + fac) / 2., mtot * (1. - fac) / 2.)
    tau = calc_chirptime(m1, m2, f_min=f_low)+8 
    df = 1/2**np.ceil(np.log2(tau))
    hp, hc = get_lensed_fd_waveform(approx, m1, m2, dL, spin1z, spin2z, iota, phi0, df, f_low, f_upp, M_lz, y_l, intrp_hyp1f1)
    return hp

#def calc_mismatch_lensed_waveforms(approx, mchirp, eta, eff_spin, f_low, f_upp, Sh, f_Sh, M_lz, y_l, intrp_hyp1f1=None):
#def calc_mismatch_lensed_waveforms(approx, mchirp, eta, spin1z, spin2z, f_low, f_upp, Sh, f_Sh, M_lz, y_l, intrp_hyp1f1=None):
def calc_mismatch_lensed_waveforms(h, approx, mchirp, eta, spin1z, spin2z, f_low, f_upp, Sh, f_Sh,\
                                   M_lz=m_r, y_l=y_r, intrp_hyp1f1=None):
...
#        log_mismatch = 1e6 
    if isnan(m1) or isnan(m2) or isnan(spin1z) or isnan(spin2z) or m1 < 2. or m2 < 2 or m1/m2 < 1./18 or\
    m1/m2 > 18 or m1+m2 > 800 or spin1z < -0.99 or spin1z > 0.99 or spin2z < -0.99 or spin2z > 0.99:
        log_mismatch = 1e6    
    else:
...
#                                         delta_f=df, low_freq_cutoff=f_low)
        Sh = psd.read.from_numpy_arrays(freq_data=f_Sh, noise_data=Sh, length=flen, delta_f=df, low_freq_cutoff=f_low)
...
#        log_mismatch = calc_mismatch(hp, approx, mchirp, eta, spin1z, spin2z, df, f_low, f_upp, Sh)
        match, shift_idx = filter.matchedfilter.match(h, hp, psd=Sh, low_frequency_cutoff=f_low,\
                                                      high_frequency_cutoff=f_upp, v1_norm=None, v2_norm=None)
        log_mismatch = np.log10(1-round(match,10))
...
#   res = minimize(fun_log_mismatch, (mchirp_0, eta_0, eff_spin_0), method='Nelder-Mead', options={'adaptive':True}) 
   res = minimize(fun_log_mismatch, (mchirp_0, eta_0, spin1z_0, spin2z_0), method='Nelder-Mead', options={'adaptive':True})    
   
   return res.fun, res['x']


#def calc_ff_lensed_waveforms(approx, mchirp, eta, spin1z, spin2z, f_low, f_upp, Sh, f_Sh, M_lz, y_l, intrp_hyp1f1=None, N_iter=1):
...
##        log_mismatch = 1e6   
#    if m1 < 2. or m2 < 2 or m1/m2 < 1./18 or m1/m2 > 18 or m1 > 600 or m2 > 600 or m1+m2 > 700 or spin1z < -0.99 or spin1z > 0.99 or\
#        spin2z < -0.99 or spin2z > 0.99:
#        log_mismatch = 1e6  
#    else:
#        
#        # determine freq resolution using chirp time
...
##        fun_log_mismatch = lambda x: calc_mismatch(hp, approx, x[0], x[1], x[2], x[3], df, f_low, f_upp, Sh)
...
#            log_mismatch, min_arr = np.min(np.vectorize(minimize_mismatch)(fun_log_mismatch, mchirp_0, eta_0, eff_spin_0))

#        else: 
#            log_mismatch, min_arr = minimize_mismatch(fun_log_mismatch, mchirp, eta, eff_spin)
##            log_mismatch, min_arr = minimize_mismatch(fun_log_mismatch, mchirp, eta, spin1z, spin2z)

#    print('log_mismatch = {}, min_arr = {}'.format(log_mismatch, min_arr))
#                
#    return log_mismatch, min_arr


def calc_ff_lensed_waveforms(approx, mchirp, eta, spin1z, spin2z, f_low, f_upp, Sh, f_Sh, M_lz, y_l,\
                             intrp_hyp1f1=None, N_iter=1):
...
#     eff_spin = (m1*spin1z + m2*spin2z)/(m1+m2)
    ...
    # apply some boundary 
#    if m1 < 2. or m2 < 2 or m1/m2 < 1./18 or m1/m2 > 18 or m1 > 600 or m2 > 600 or m1+m2 > 700 or\
#    eff_spin < -0.99 or eff_spin > 0.99:
#        log_mismatch = 1e6   
    if m1 < 2. or m2 < 2 or m1/m2 < 1./18 or m1/m2 > 18 or m1 > 600 or m2 > 600 or m1+m2 > 700 or\
        spin1z < -0.99 or spin1z > 0.99 or spin2z < -0.99 or spin2z > 0.99:
        log_mismatch = 1e6  
    else:
        ...
        # function to be minimized 
#        fun_log_mismatch = lambda x: calc_mismatch_lensed_waveforms(hp, approx, x[0], x[1], x[2], df,\
#                                                                    f_low, f_upp, Sh)
        fun_log_mismatch = lambda x: calc_mismatch_lensed_waveforms(hp, approx, x[0], x[1], x[2], x[3],\
                                                                    f_low, f_upp, Sh, f_Sh)
...
def hp_hc(approx, mchirp, eta, spin1z, spin2z, f_low, f_upp, Sh, f_Sh, M_lz=m_i, y_l=y_i, intrp_hyp1f1=None):
    ...
    tau = calc_chirptime(m1, m2, f_min=f_low)+8 
    df = 1/2**np.ceil(np.log2(tau))
    hp, hc = get_lensed_fd_waveform(approx, m1, m2, dL, spin1z, spin2z, iota, phi0, df, f_low, f_upp, M_lz,\
                                    y_l, intrp_hyp1f1)
    return hp
...
path_plot = '../../../sim_data/soummyadip_o3b/code_results_review/'
...
print(len(f_H1), len(asd_H1), m_i, y_i, m_r, y_r)

#Sh_H1 = aLIGOZeroDetHighPower(length=len(f_H1), delta_f=f_H1[1]-f_H1[0], low_freq_cutoff=0)
#Sh_L1 = aLIGOZeroDetHighPower(length=len(f_L1), delta_f=f_L1[1]-f_L1[0], low_freq_cutoff=0)
#Sh_V1 = aLIGOZeroDetHighPower(length=len(f_V1), delta_f=f_V1[1]-f_V1[0], low_freq_cutoff=0)
...
if mkplots == True: 
    ...
    plt.savefig(path_plot+'psds_%s.pdf' %(runtag))
...
m1_det_s = 30
m2_det_s = 30
mc_det_s = (m1_det_s*m2_det_s)**(3./5)/(m1_det_s+m2_det_s)**(1./5)
eta_s = m1_det_s*m2_det_s/(m1_det_s+m2_det_s)**2
...
if mkplots==True: 
    ...
    plt.ylabel('$P(y)$')
    plt.tight_layout()
    plt.savefig(path_plot+'dist_Mlz_y_%s_v2.pdf' %(runtag))
...
if mkplots==True: 
    ...
    plt.colorbar()
    plt.xlabel('$\log_{10}~ M_{l}^z~(\mathrm{M_\odot})$')
    plt.ylabel('$y$')
    ...
    plt.savefig(path_plot+'lensing_td_chirptime_%s_v2.png' %(runtag), dpi=300)
...
hp_H1 = hp_hc(approx, mc_det_s, eta_s, spin1_s, spin2_s, f_low, f_upp, Sh_H1, f_H1)

#log_mismatch_H1 = np.vectorize(calc_mismatch_lensed_waveforms, otypes=[object], excluded={6,7})\
#                              (approx, mc_det_s, eta_s, spin1_s, f_low, f_upp, Sh_H1, f_H1,  
#                               M_lz, y_l, intrp_hyp1f1=None)

log_mismatch_H1 = np.vectorize(calc_mismatch_lensed_waveforms, otypes=[object], excluded={0,8,9})\
                              (hp_H1, approx, mc_det_s, eta_s, spin1_s, spin2_s, f_low, f_upp, Sh_H1, f_H1, intrp_hyp1f1=intrp_hyp1f1)

match_H1 = 1 - 10**log_mismatch_H1

print('match_H1 = {}'.format(match_H1))
...
log_one_minus_ff_H1, min_arr_H1 = np.vectorize(calc_ff_lensed_waveforms, otypes=[object], excluded={7,8})\
                                              (approx, mc_det_s, eta_s, spin1_s, spin2_s, f_low, f_upp,\
                                               Sh_H1, f_H1, M_lz=m_i, y_l=y_i, intrp_hyp1f1=intrp_hyp1f1)

ff_H1 = 1 - 10**log_one_minus_ff_H1

print('ff_H1 = {}, min_arr_H1 = {}'.format(ff_H1, min_arr_H1))


hp_L1 = hp_hc(approx, mc_det_s, eta_s, spin1_s, spin2_s, f_low, f_upp, Sh_L1, f_L1)

#log_mismatch_L1 = np.vectorize(calc_mismatch_lensed_waveforms, otypes=[object], excluded={6,7})\
#                          (approx, mc_det_s, eta_s, spin1_s, f_low, f_upp, Sh_L1, f_L1,  
#                           M_lz, y_l, intrp_hyp1f1=None)

log_mismatch_L1 = np.vectorize(calc_mismatch_lensed_waveforms, otypes=[object], excluded={0,8,9})\
                              (hp_L1, approx, mc_det_s, eta_s, spin1_s, spin2_s, f_low, f_upp, Sh_L1, f_L1, intrp_hyp1f1=intrp_hyp1f1)

match_L1 = 1 - 10**log_mismatch_L1

print('match_L1 = {}'.format(match_L1))
...
log_one_minus_ff_L1, min_arr_L1 = np.vectorize(calc_ff_lensed_waveforms, otypes=[object], excluded={7,8})\
                                              (approx, mc_det_s, eta_s, spin1_s, spin2_s, f_low, f_upp,\
                                               Sh_L1, f_L1, M_lz=m_i, y_l=y_i, intrp_hyp1f1=intrp_hyp1f1)

ff_L1 = 1 - 10**log_one_minus_ff_L1

print('ff_L1 = {}, min_arr_L1 = {}'.format(ff_L1, min_arr_L1))


hp_V1 = hp_hc(approx, mc_det_s, eta_s, spin1_s, spin2_s, f_low, f_upp, Sh_V1, f_V1)

#log_mismatch_V1 = np.vectorize(calc_mismatch_lensed_waveforms, otypes=[object], excluded={6,7})\
#                          (approx, mc_det_s, eta_s, spin1_s, f_low, f_upp, Sh_V1, f_V1,  
#                           M_lz, y_l, intrp_hyp1f1=None)

log_mismatch_V1 = np.vectorize(calc_mismatch_lensed_waveforms, otypes=[object], excluded={0,8,9})\
                              (hp_V1, approx, mc_det_s, eta_s, spin1_s, spin2_s, f_low, f_upp, Sh_V1, f_V1, intrp_hyp1f1=intrp_hyp1f1)

match_V1 = 1 - 10**log_mismatch_V1

print('match_V1 = {}'.format(match_V1))
...
log_one_minus_ff_V1, min_arr_V1 = np.vectorize(calc_ff_lensed_waveforms, otypes=[object], excluded={7,8})\
                                              (approx, mc_det_s, eta_s, spin1_s, spin2_s, f_low, f_upp,\
                                               Sh_V1, f_V1, M_lz=m_i, y_l=y_i, intrp_hyp1f1=intrp_hyp1f1)

ff_V1 = 1 - 10**log_one_minus_ff_V1

print('ff_V1 = {}, min_arr_V1 = {}'.format(ff_V1, min_arr_V1))
...
if mkplots==True: 
    ...
    plt.xlabel('$\log_{10}~M_\ell^z~(\mathrm{M_\odot})$')
    plt.ylabel('$y$')
    ...
    plt.clim(0, 0.5)
    plt.ylim(min(y_l), max(y_l))
    plt.xlabel('$\log_{10}~M_\ell~(\mathrm{M_\odot})$')
    plt.ylabel('$y$')
    plt.colorbar(label='$\ln~\mathcal{B}^\mathrm{L}_\mathrm{U}$')
    plt.tight_layout()
    plt.savefig(path_plot+'blu_scatter_plot_Ml_y_%s.png' %(runtagff), dpi=300)


## save the Blu data 
#outfile = '%s/machodm_ffdata_%s_fdm_%.1f_m_lens_%.1e' %(outdir, runtag, f_dm, m_lens)
...
#outfile = '%s/machodm_ffdata_%s' %(outdir, runtag)
outfile = '%s/machodm_ffdata_%s_modified' %(outdir, runtag)

#np.savez(outfile, m1_det_s=m1_det_s, m2_det_s=m2_det_s, spin1_s=spin1_s, spin2_s=spin2_s, injected_M_lz=M_lz, injected_y=y_l,\
#         match_H1=match_H1, match_L1=match_L1, match_V1=match_V1, ff_H1=ff_H1, ff_L1=ff_L1, ff_V1=ff_V1,\
#         min_arr_H1=min_arr_H1, min_arr_L1=min_arr_L1, min_arr_V1=min_arr_V1, m_i=m_i, m_r=m_r, y_i=y_i, y_r=y_r)

np.savez(outfile, m1_det_s=m1_det_s, m2_det_s=m2_det_s, spin1_s=spin1_s, spin2_s=spin2_s,\
         match_H1=match_H1, match_L1=match_L1, match_V1=match_V1, ff_H1=ff_H1, ff_L1=ff_L1, ff_V1=ff_V1,\
         min_arr_H1=min_arr_H1, min_arr_L1=min_arr_L1, min_arr_V1=min_arr_V1, m_i=m_i, m_r=m_r, y_i=y_i, y_r=y_r)

print('ff data is saved.')
