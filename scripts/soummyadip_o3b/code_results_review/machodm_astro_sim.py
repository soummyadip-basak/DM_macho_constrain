...
parser.add_argument('--approx', metavar='approx', type=str, help='waveform approximant')
parser.add_argument('--f_upp', metavar='f_upp', type=float, help='upper frequency in SNR calculation in Hz')
...
def calc_redshift_dist(zs, zs_dist, zs_max):
    ...
    if zs_dist == 'uniform': # uniform in comoving volume 
        ...
        z, Pz_Dominik, Pz_Belczynski, Pz_PopIII, Pz_primo = np.loadtxt('../../../data/redshift_data/z_PDF.dat', unpack=True, skiprows=1)
        ...
        Ez = lcdm.hubble_normalized_z(zs)
        kappa = 2.7 
        Pb_z = (1+z)**kappa*4*np.pi*c_km*chi_s**2/(1+zs)/H_0/Ez 
...
for zdist in ['uniform', 'Dominik', 'Belczynski', 'O3cosmo', 'Madau-Dickinson']:
...
path_snr = '/home1/soummyadip.basak/lensing/DM_macho_constrain/sim_data/soummyadip_o3b/code_results_review/'

data_1024 = np.load(path_snr+'machodm_lensing_astro_sim_Madau-Dickinson_IMRPhenomD_plawidx_power_law_peak_zsmax'+\
                    '_1.5_y0_5.0_H1L1V1_O3b_psd_2022-01-10_Nsim_1e+05_fupp_1024.npz')

m1_s = data_1024['m1_s']
m2_s = data_1024['m2_s']
q_s = m2_s/m1_s
spin1_s = data_1024['spin1_s']
spin2_s = data_1024['spin2_s']
z_s = data_1024['z_s']
ldist_s = data_1024['ldist_s']
ra_s = data_1024['ra_s']
dec_s = data_1024['dec_s']
sindec_s = np.sin(dec_s)
pol_s = data_1024['pol_s']
iota_s = data_1024['iota_s']
cosiota_s = np.sin(iota_s)
phi0_s = data_1024['phi0_s']
t0_gps = data_1024['t0_gps']
m1z_s, m2z_s = m1_s*(1+z_s), m2_s*(1+z_s)  
...
def calc_snr(m1, m2, spin1x, spin2x, dL, ra, dec, pol, iota, phi0, f_low, f_upp, df, t0_gps, approx): 
    """ calculate the optimal SNR at H1, L1, V1 detectors
    Note: Requires the detector objects H1, L1, V1 to be set globally. Also psd_H1, psd_L1, psd_V1
    """
    
    # generate waveform 
    hp, hc = get_fd_waveform(approximant=approx, mass1=m1, mass2=m2, distance=dL, spin1x=spin1x, spin2x=spin2x, inclination=iota,\
                             coa_phase=phi0, delta_f=df, f_lower=f_low, f_final=f_upp)
...
## save the data 
#outfile = '../../../sim_data/soummyadip_o3b/code_results_review/machodm_lensing_astro_sim_%s' %(runtag)
...
#outfile = '../../../sim_data/soummyadip_o3b/code_results_review/machodm_lensing_astro_sim_%s_lw' %(runtag)
...
# save the data 
outfile = '../../../sim_data/soummyadip_o3b/code_results_review/machodm_lensing_astro_sim_%s_spin_x' %(runtag)
np.savez(outfile, m1_s=m1_s, m2_s=m2_s, spin1_s=spin1_s, spin2_s=spin2_s, z_s=z_s, ldist_s=ldist_s, 
         ra_s=ra_s, dec_s=dec_s, pol_s=pol_s, iota_s=iota_s, phi0_s=phi0_s, t0_gps=t0_gps, 
         snr_L1=snr_L1, snr_H1=snr_H1, snr_V1=snr_V1, det_idx=det_idx, df=df, f_low=f_low, f_upp=f_upp, 
         z_l=z_l, lens_idx_dic=lens_idx_dic, z_l_dic=z_l_dic)
...
# save only detected events 
outfile = '../../../sim_data/soummyadip_o3b/code_results_review/machodm_lensing_astro_sim_%s_lw_spin_x' %(runtag)
np.savez(outfile, m1_s=m1_s[det_idx], m2_s=m2_s[det_idx], spin1_s=spin1_s[det_idx], spin2_s=spin2_s[det_idx], 
         z_s=z_s[det_idx], ldist_s=ldist_s[det_idx], ra_s=ra_s[det_idx], dec_s=dec_s[det_idx], 
         pol_s=pol_s[det_idx], iota_s=iota_s[det_idx], phi0_s=phi0_s[det_idx], t0_gps=t0_gps[det_idx], 
         snr_L1=snr_L1[det_idx], snr_H1=snr_H1[det_idx], snr_V1=snr_V1[det_idx], df=df, f_low=f_low, f_upp=f_upp, 
