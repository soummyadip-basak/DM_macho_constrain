...
def lens_ampl_function_geom_optics(f, M_lz, y): 
...
def lens_ampl_function(f, M_lz, y, intrp_hyp1f1=None):
    ...
    if y > 3:
        F_f = lens_ampl_function_geom_optics(f, M_lz, y)
    ...
    # if y <= 3, use a combination of wave optics (low freq) and geom optics (high freq)    
        ...
        if len(f_2) != 0:
            ...
            F_g = lens_ampl_function_geom_optics(f_2, M_lz, y)
            F_f = np.concatenate((F_w,F_g))

        else:
            F_f = lens_ampl_function_wave_optics(f_1, M_lz, y, intrp_hyp1f1=intrp_hyp1f1)
...
print('log_one_minus_ff_L1 calculation is over.')
...
print('log_one_minus_ff_V1 calculation is over.')
...
print('Blu scatter plot got saved')
